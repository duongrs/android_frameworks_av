...
status_t ACodec::configureCodec(
    ...
    if (video) {
        ...
        CHECK(msg->findInt32("channel-count", &numChannels));
        CHECK(msg->findInt32("sample-rate", &sampleRate));
        setMPGAudioFormatNexell(encoder, numChannels, sampleRate);
...
status_t ACodec::setMPGAudioFormatNexell(bool encoder, int32_t numChannels, int32_t sampleRate)
{
    status_t err = setupRawAudioFormat( encoder ? kPortIndexInput : kPortIndexOutput, sampleRate, numChannels);
    ALOGD("setMPGAudioFormatNexell() setupRawAudioFormat() return (%d)", err);
    if (err != OK) {
        return err;
    }
